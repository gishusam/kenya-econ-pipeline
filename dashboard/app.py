...
def load_dashboard_data():
    ...
    pipeline = query_dataframe(
        client,
        f"SELECT * FROM `{project_id}.marts.pipeline_status`",
    )
...
with c4:
    st.metric(
        "GDP · current KES",
        f"KES {gdp_lcu / 1e12:.2f}T" if gdp_lcu is not None else "—",
    )
...
with left:
    ...
    gdp_history = history[
        history["indicator_code"] == "REAL_GDP_GROWTH"
    ].copy()
...
with right:
    ...
    cpi_history = history[
        history["indicator_code"] == "CPI_INFLATION_YOY"
    ].copy()
...
if health.empty:
    ...
    health_display = prepare_health_for_display(health)

    health_display["Status"] = health_display[
        "freshness_status"
    ].map(
    ...
    columns = [
        ...
        if column in health_display.columns
    ]

    st.dataframe(
        health_display[columns].rename(
            columns={
                "source": "Source",
...
if not pipeline.empty:
    ...
    completed_text = (
        format_eat_timestamp(latest_run.get("completed_at"))
        or "unknown"
    )

    st.caption(
        f"Pipeline: "
        f"{str(latest_run.get('status', 'unknown')).upper()} · "
        f"last completed {completed_text} · "
        f"rows added "
        f"{int(latest_run.get('rows_inserted', 0) or 0)}"
    )
